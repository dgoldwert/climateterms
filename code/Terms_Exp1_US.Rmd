---
title: "Climate Terms Experiment 1 US-only Analyses"
...
generate_lm_model_and_latex_output(M, "willingness", "../overleaf/tabs_US/main.tex")
print(summary(M))
confint(M)
...
dfc_US <- na.omit(df_long_US)

# Calculate 'Ideology' as the average of 'Politics2_1' and 'Politics2_9'
dfc_US$Ideology <- (dfc_US$Politics2_1 + dfc_US$Politics2_9) / 2
...
dfc_US$Ideology_ms[is.na(dfc_US$Politics2_1)] <- NA
...
```{r SES}
M <- lm(willingness ~ Term*SES, data = dfc_US)
...
```{r edu}
M <- lm(willingness ~ Term*Education, data = dfc_US)
