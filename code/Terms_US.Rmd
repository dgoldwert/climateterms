...
dfc_US <- dfc_US[dfc$Gender %in% c(1, 2), ]

# Create continuous ideology variable
dfc_US$dfc_US <- (dfc_US$Politics2_1 + dfc_US$Politics2_9) / 2

# Create median split ideology variable
new <- list()
for (country in unique(dfc_US$Country)) {
  sub <- dfc_US[dfc_US$Country == country, ] # Filter for rows in the current country
...
dfc_US <- do.call(rbind, new)
```

```{r ideology (continuous)}
#continuous ideology variable
M <- lmer(willingness ~ Term*ide + (1 | Country), data = dfc)
generate_model_and_latex_output(M, "willingness", "../overleaf/tabs_US/main_ide_cont.tex")
print(summary(M))
confint(M)
...
M <- lmer(willingness ~ Term*Ideology + (1 | Country), data = dfc_US)
generate_model_and_latex_output(M, "willingness", "../overleaf/tabs_US/main_ide_ms.tex")
print(summary(M))
confint(M)
...
```{r SES}
M <- lmer(willingness ~ Term*MacArthur_SES + (1 | Country), data = dfc_US)
generate_model_and_latex_output(M, "willingness", "../overleaf/tabs_US/main_SES.tex")
print(summary(M))
confint(M)
...
```{r gender}
M <- lmer(willingness ~ Term*Gender + (1 | Country), data = dfc_US)
generate_model_and_latex_output(M, "willingness", "../overleaf/tabs_US/main_gender.tex")
print(summary(M))
confint(M)
...
```{r age}
M <- lmer(willingness ~ Term*Age + (1 | Country), data = dfc_US)
generate_model_and_latex_output(M, "willingness", "../overleaf/tabs_US/main_age.tex")
print(summary(M))
confint(M)
...
```{r edu}
M <- lmer(willingness ~ Term*Edu + (1 | Country), data = dfc_US)
generate_model_and_latex_output(M, "willingness", "../overleaf/tabs_US/main_edu.tex")
print(summary(M))
confint(M)
