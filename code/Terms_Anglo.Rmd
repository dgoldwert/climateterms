---
title: "Climate Terms Anglosphere-only Analyses"
...
df_long_anglo <- df_long1 %>%
  filter(Country %in% countries1)

# Remove rows with missing data
dfc_anglo <- df_long_anglo %>%
  drop_na()

# Remove participants whose Gender is not 1 or 2
dfc_anglo <- dfc_anglo[dfc$Gender %in% c(1, 2), ]

# Create continuous ideology variable
dfc_anglo$dfc_anglo <- (dfc_anglo$Politics2_1 + dfc_anglo$Politics2_9) / 2

# Create median split ideology variable
new <- list()
for (country in unique(dfc_anglo$Country)) {
  sub <- dfc_anglo[dfc_anglo$Country == country, ] # Filter for rows in the current country
...
```{r SES}
M <- lmer(willingness ~ Term*MacArthur_SES + (1 | Country), data = dfc_anglo)
...
```{r edu}
M <- lmer(willingness ~ Term*Edu + (1 | Country), data = dfc_anglo)
