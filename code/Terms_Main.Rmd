...
generate_model_and_latex_output <- function(model, response_var, output_file_tex) {
  ...
  library(xtable)
  library(lmerTest)  # Ensure lmerTest is loaded for 'df' in summary
  ...
  # Remove "as.factor(Term)" from the Condition names and replace underscores with spaces
  Condition <- gsub("as\\.factor\\(Term\\)", "", fe_names)
