---
title: "Climate Terms Experiment 1 Main Analyses"
...
generate_tukey_latex_output <- function(tukey_test, output_file_tex) {
  ...
  # Set alignment for columns (length should be ncol + 1)
  align_spec <- c("l", "l", "r", "r", "r", "l", "r")  # Adjusted for 6 columns
...
dfc$Ideology <- (dfc$Politics2_1 + dfc$Politics2_9) / 2
...
for (country in unique(dfc$Country)) {
  ...
  country_median <- median(sub$Ideology, na.rm = TRUE) # Calculate the median of ide within this country
  sub$Ideology_ms <- as.numeric(sub$Ideology > country_median) # Perform the median split and convert to numeric (0 or 1)
...
M <- lmer(willingness ~ Term*Ideology_ms + (1 | Country), data = dfc)
...
```{r SES}
M <- lmer(willingness ~ Term*SES + (1 | Country), data = dfc)
...
```{r edu}
M <- lmer(willingness ~ Term*Education + (1 | Country), data = dfc)
