...
# Calculate 'ide' as the average of 'Politics2_1' and 'Politics2_9'
dfc$Ideology <- (dfc$Politics2_1 + dfc$Politics2_9) / 2

# Perform per-country median split
dfc <- dfc %>%
  group_by(Country) %>%
  mutate(
    # Compute Ideology: 1 if 'Ideology' > median 'Ideology' within country, else 0
    Ideology_ms = as.numeric(Ideology > median(Ideology, na.rm = TRUE))
  ) %>%
  ungroup()

# Ensure 'Ideology' is NA where 'Politics2_1' is NA
dfc$Ideology_ms[is.na(dfc$Politics2_1)] <- NA
